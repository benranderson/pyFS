...
class Model:
    ...
    def create_beam_element(self, number=0, N1=0, N2=0, N3=0, rotation=0, geometry=0, material=0,
                                               relZ=0, relzY=0, taper=0, type=0, CO=0, bend_radius=0):
        if number == 0:
    ...
    def _create_model_files(self):
        self.write_MDL_file()    
        winfram_path = os.path.join(self._install_directory, 'system\winfram.exe')
        subprocess.call([winfram_path, 'I'])

    def write_MDL_file(self):
        path = os.path.join(self.path, self.name + '.mdl')
        with open(path, 'w') as MDL:
            MDL.writelines('N,' + n.number + ',' + n.x + ',' + n.y + ',' + n.z + ',' + n.CSYS for n in self.nodes)
            MDL.writelines('E,' + e.number + ',' + e.N1 + ',' + e.N2 + ',' + e.N3 + ',' + e.rotation + ',' + e.geometry
                                   + ',' + e.material + ',' + e.relZ + ',' + e.relY + ',' + e.taper + ',' + e.type + ',' + e.CO
...
class Node:
    ...
    def __repr__(self):
        return 'Node {0} <{1}, {2}, {3}> CSYS={4}'.format(self.number, self.x, self.y, self.z, self.CSYS)
...
class BeamElement:
    ...
    def __repr__(self):
        return 'Beam Element {0} <N1 = {1}, N2 = {2}>'.format(self.number, self.N1, self.N2)
